...
namespace MathOptimizer.Methods.MultyDimensional
{
    ...
    /// optimization method
    /// </summary>
    class PatternSearch : MultyDimMethod
    {
        public override void run(Function f, Parameters parameters)
        {
            // Get input parameters
            Vector startPoint      = parameters.Input.StartVecPoint;
            ...
            double norm = Vector.Norm(startPoint);
            if (norm != 0)
            {
                startStep *= norm;
            }
            ...
            parameters.Output.ResultVecPoint = x1;
